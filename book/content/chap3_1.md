...
:::{raw} html
<iframe src="../content/objective_explorer.html" width="100%" height="550px" frameborder="0" style="border: none;"></iframe>
:::
...
Microscope manufacturers follow a standardized color-coding system to help users quickly identify objectives by magnification and immersion type. Two colored rings near the bottom of the barrel encode this information ({numref}`Fig. {number} <objective_colors>`). The upper ring (thicker, closer to the barrel text) indicates the magnification. The lower ring (thinner, closer to the front element) indicates the immersion medium; dry objectives have no immersion ring at all.

```{figure} ../figures/objective_colors.png
---
width: 80%
name: objective_colors
align: center
---
Standardized color coding for microscope objectives. The upper ring indicates the magnification, and the lower ring indicates the immersion medium.
```

The magnification color code is:
...
The immersion medium color code is:
...
Dry objectives have no immersion ring.

```{tip}
At the microscope, the quickest way to grab the right objective is to look for the magnification color ring. After a few weeks in the lab, you will recognize them without thinking. The immersion ring is especially useful to avoid the common mistake of using an oil objective dry (which produces a terrible image) or dripping oil onto a dry objective (which makes a mess but, fortunately, is easy to clean).
```

In the next section, we will address the question that naturally follows: if we can keep multiplying magnifications, why can't we just use a 10× objective with a 1000× eyepiece and see atoms? The answer, as we will discover, has nothing to do with lenses and everything to do with the wave nature of light.
